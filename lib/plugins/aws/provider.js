  ...
  memoizeeMethods({
    ...
    dockerLoginToEcr: d(
      async function () {
        ...
        try {
          const dockerLoginPromise = spawnExt('docker', dockerArgs);
          dockerLoginPromise.child.stdin.write(decodedAuthToken);
          dockerLoginPromise.child.stdin.end();
          await dockerLoginPromise;
          process.stdout.write(dockerLoginPromise.stdBuffer);
        } catch (err) {
          throw new this.serverless.classes.Error(
    ...
    resolveImageUriAndShaFromPath: d(
      async function (imageName, imagePath) {
        ...
        try {
          ...
          isDockerfileAvailable = stat.isFile();
        } catch {
        ...
        const buildDockerArgs = ['build', '-t', localTag, imagePath];
        try {
        ...
        const imageSha = pushDockerStdBuffer.toString().match(/(sha256:[a-f0-9]{64})/)[0];
        ...
        if (imageDefinedInProvider) {
          if (_.isObject(imageDefinedInProvider)) {
            if (!imageDefinedInProvider.uri && !imageDefinedInProvider.path) {
              throw new this.serverless.classes.Error(
                `Either "uri" or "path" property needs to be set on image "${image}"`
              );
            }
            if (imageDefinedInProvider.uri && imageDefinedInProvider.path) {
              throw new this.serverless.classes.Error(
                `Either "uri" or "path" property (not both) needs to be set on image "${image}"`
              );
            }
            if (imageDefinedInProvider.path) {
              return await this.resolveImageUriAndShaFromPath(image, imageDefinedInProvider.path);
            }
          ...
          return await this.resolveImageUriAndShaFromPath(image, imageDefinedInProvider);
        }
        if (!isEcrUri(image)) {
          throw new this.serverless.classes.Error(
            `Referenced "${image}" not defined in "provider.ecr.images"`
          );
        }
        return await this.resolveImageUriAndShaFromUri(image);
      },
      { promise: true }
    ),
  })
